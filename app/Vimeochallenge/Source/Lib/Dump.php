 ...
 * Please enter your settings in the the configue.php file in your root dir
 * The Database will be created using the DBNAME Constant provided.
 * All tables will also be created.
 * Make sure to provide a DB User with proper Create Access
 * Once Database created and connection has been established,
 * we will read through data.dump file provided line by line.
 * I decided to enter all country codes from the REGISTER inserts
 * in its own 'countries' table.
 * Country is now referenced as a integer identifier and better for indexing.
 * Likes, watch and other totals are now stored as static value within
 * USER and VIDEOS table.
...
class Dump
{
    ...
    private $_recordTypeSql = [
            'REGISTER' =>   'INSERT INTO users '
                            .'(user_id,created,country_id,ip_address) VALUES ',
            'UPLOAD' =>     'INSERT INTO videos '
                            .'(video_id,upload_date,user_id) VALUES ',
            'WATCH' =>      'INSERT INTO videos_watch_log '
                            .'(video_id,watch_date,user_id) VALUES ',
            'LIKE' =>       'INSERT INTO videos_likes_log '
                            .'(video_id,like_date,user_id) VALUES ',
    ...
    final private function logError($info)
    {
        ...
        fwrite($errorFile,$errdate."\t".$this->_line."\t".$info.PHP_EOL);
        fclose($errorFile);
    }
    /**
    ...
    final private function runSql(string $sql,array $params=NULL)
    {
        ...
        try {
            ...
            if ($STH->errorCode() != 0) {
                ...
                $erCode = $STH->errorCode();
                throw new Exception('Database error:'.$erCode.'-'.$err_msg[2]);
            } else {
    ...
    /**
     * Method used to determine which INSERT statement to used based on type.
     */
    final private function insertSQL($type,$final=false)
    {
        ...
        /**
         * Check and see if it is a FINAL request. 
         * If so, submit whatever sql is available.
         */
        if(     $this->$paramName['total'] >= self::CHUNKSIZE
            ||  ($final && isset($this->$paramName['records'])
            &&  count($this->$paramName['records'])>0)
        ) {
            $sqlStmt = $this->_recordTypeSql[$type];
            $newInsert = $this->runSql($sqlStmt,$this->$paramName['records']);
            if ($newInsert['status']) {
                $totalInserts = $this->$paramName['total'];
                echo $totalInserts.$type.' CHUNK INSERTS Added'.PHP_EOL;
                $this->_successful += $totalInserts;
            }
    ...
    final public function processRecord($line)
    {
        ...
        /**
         * If REGISTER type, let's first check to see if country code exists
         * in '_countries' property
         */
        if (    $record['1'] == 'REGISTER'
            &&  !isset($this->_countries[$record[3]])
        ) {
            echo 'New Country '. $record[3], ' Found', PHP_EOL;
            $sqlStmt = $this->_recordTypeSql['COUNTRY'];
            $countryInsert = $this->runSql($sqlStmt,[$record[3]]);
            
            if ($countryInsert['status']) {
                $cMsg = $countryInsert['message'];
                echo 'New Country ',$record[3],' Added ID(',$cMsg, ')',PHP_EOL;
                $this->_countries[$record[3]] = $cMsg;
            } else {
                $ok = false;
            }
        ...
        if ($ok) {
            ...
            $newInsert;
            $params;
            $timestamp = date('Y-m-d H:i:s',strtotime($record[0]));
    ...
    final private function init()
    {
        ...
        if ($handle) {
            ...
            while (($data = fgets($handle, 600)) !== FALSE) {
                ...
                /**
                 * lets get the second item and determine
                 * what type of actions this was
            ...
            if (!feof($handle)) {
                echo    'Data Dump ended before the End of File('
                        .self::MAXREAD.' rows processed).',PHP_EOL;
            }
            fclose($handle);
        }
        ...
        $updateUsersSql =   'UPDATE users u '
                            .'LEFT JOIN '
                            .'(SELECT user_id ups_id, count(*) ups_total '
                            .'  FROM videos GROUP BY ups_id) ups '
                            .'ON u.user_id = ups.ups_id '
                            .'LEFT JOIN '
                            .'(SELECT user_id likes_id, count(*) likes_total '
                            .'  FROM videos_likes_log GROUP BY likes_id) likes '
                            .'ON u.user_id = likes.likes_id '
                            .'LEFT JOIN '
                            .'(SELECT user_id watch_id, count(*) watch_total '
                            .'  FROM videos_watch_log GROUP BY watch_id) watched '
        ...
        $updateVidsSql =    'UPDATE videos v '
                            .'LEFT JOIN '
                            .'  (SELECT video_id likes_id, count(*) likestotal'
                            .'      FROM videos_likes_log GROUP BY likes_id'
                            .'  ) lt '
                            .'  ON v.video_id = lt.likes_id '
                            .'LEFT JOIN '
                            .'  (SELECT video_id watch_id, count(*) watchtotal'
                            .'      FROM videos_watch_log GROUP BY watch_id'
                            .'  ) wt '
                            .'  ON v.video_id = wt.watch_id '
                            .'SET '
                            .'  v.likes = coalesce(likestotal,0), '
                            .'  v.watched = coalesce(watchtotal,0)';
        ...
        /**
         * Get top 5 videos from user table.
         * No error handling for now i can add it later.
         */       
        ...
        while ($row = $STH->fetch(PDO::FETCH_ASSOC)) {
            ...
            $id = $row['video_id'];
            $addTopSQL =    'INSERT INTO most (type,id,sortorder) '
                            .'VALUES ("mwv",'.$id.','.$i.')';
